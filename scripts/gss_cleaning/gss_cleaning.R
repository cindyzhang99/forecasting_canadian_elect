...
raw_data <- read_csv("inputs/gss/gss_2017.csv")
dict <- read_lines("scripts/gss_cleaning/gss_dict.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("scripts/gss_cleaning/gss_labels.txt")
...
gss <- raw_data %>% 
  select(CASEID, 
         ...
         csp_160,
         fi_110) %>% 
  mutate_at(vars(agec:fi_110), .funs = funs(ifelse(.>=96, NA, .))) %>% 
...
write_csv(gss, "inputs/gss/gss_2017_cleaned.csv")
